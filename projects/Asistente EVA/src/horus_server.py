...
async def horus_service_worker(request):
    config = public_firebase_web_config(request.app["context"].config)
    firebase_config = config.get("firebaseConfig") if config.get("configured") else None
    script = HORUS_SW_PATH.read_text(encoding="utf-8").replace(
        "{{FIREBASE_CONFIG_JSON}}",
        json.dumps(firebase_config, ensure_ascii=False) if firebase_config else "null",
    )
    return web.Response(
        text=script,
...
async def horus_firebase_web_config(request):
    return web.json_response(public_firebase_web_config(request.app["context"].config))
...
async def start_horus_server(context):
    ...
    app.router.add_get("/api/firebase-web-config", horus_firebase_web_config)
