...
async def index(request):
    ...
    ws_url = f"ws://{request.host.split(':')[0]}:{context.ws_port}"
...
async def get_notification(request):
    ...
    notification = db.notifications.get_oldest_unread(username)
...
async def mark_notification_read(request):
    ...
    try:
        ...
        return web.json_response({
            "ok": False,
            "mensaje": "Falta id de notificación.",
        }, status=400)

    marked = db.notifications.mark_read(username, notification_id)

    return web.json_response({
        "ok": marked,
    }, status=200 if marked else 404)


async def save_push_token(request):
    db = request.app["context"].db
    username = normalize_user_or_broadcast(db, request.match_info["username"])
    ...
    if not token:
        return web.json_response({
            "ok": False,
            "mensaje": "Falta token FCM.",
        }, status=400)

    db.push_tokens.save(username, token)
...
async def test_push(request):
    ...
    tokens = db.push_tokens.get_by_username(username)
    result = await asyncio.to_thread(send_pending_notification, tokens)
    ...
    return web.json_response({
        ...
        "result": result,
    }, status=200 if result and result.get("ok") else 400)
...
async def cancel_countdown(request):
    context = request.app["context"]
    context.db.notifications.mark_unread_type_read("COUNTDOWN")
...
async def cancel_exchange_post(request):
    ...
    context.db.notifications.mark_unread_type_read("EXCHANGE_OPEN")
    await broadcast_exchange(context, exchange, "EXCHANGE_CLOSED")
...
async def cancel_door_challenge(request):
    context = request.app["context"]
    context.db.notifications.mark_unread_type_read("DOOR_CHALLENGE")
...
async def start_web_server(context):
    ...
    app.router.add_get("/notifications/{username}", get_notification)
    app.router.add_put("/notifications/{username}", mark_notification_read)
    app.router.add_put("/push-token/{username}", save_push_token)
    app.router.add_post("/api/push/test/{username}", test_push)
